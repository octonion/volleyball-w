...
from ncaa_pbp._schedule_factors_geocoding sf
--join ncaa_pbp.schools s
--  on (s.school_id)=(sf.school_id)
join ncaa_pbp.schools_divisions sd
--  on (sd.school_id)=(sf.school_id)
  on (sd.school_id,sd.year)=(sf.school_id,sf.year)
join ncaa_pbp._factors_geocoding o
  on (o.parameter,o.level::integer)=('o_div',sd.div_id)
join ncaa_pbp._factors_geocoding d
