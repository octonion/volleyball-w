...
con <- dbConnect(drv,host="localhost",port="5432",dbname="volleyball-w")
...
join ncaa_pbp.geocodes site
  on (site.school_id)=(r.location_id)
join ncaa_pbp.geocodes team
  on (team.school_id)=(r.school_id)
join ncaa_pbp.geocodes opponent
  on (opponent.school_id)=(r.opponent_id)
where
    r.year between 2009 and 2016
